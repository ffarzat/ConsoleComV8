...
namespace CodeBureau.Tests
{
	...
	public class StringEnumTests 
	{
		...
		public void TestParseExpectArgumentException()
		{
            ...
            var ex = Assert.Throws<ArgumentException>(() => StringEnum.Parse(typeof(decimal), "spoof"));
            Assert.That(ex.GetType(), Is.EqualTo(typeof(ArgumentException)));
            Assert.That(ex.Message, Is.EqualTo("Supplied type must be an Enum.  Type was System.Decimal"));
		}
		...
		{
            var ex = Assert.Throws<ArgumentException>(() => new StringEnum(typeof(string)));
		    Assert.That(ex.GetType(), Is.EqualTo(typeof (ArgumentException)));
            Assert.That(ex.Message, Is.EqualTo("Supplied type must be an Enum.  Type was System.String"));
