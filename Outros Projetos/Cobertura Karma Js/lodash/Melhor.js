...
(function () {
   ...
   */
     ...
     */
        ...
        var ArrayBuffer = context.ArrayBuffer, Reflect = context.Reflect, Set = getNative(context, 'Set'), Uint8Array = context.Uint8Array, WeakMap = getNative(context, 'WeakMap'), clearTimeout = context.clearTimeout, enumerate = Reflect ? Reflect.enumerate : undefined, getPrototypeOf = Object.getPrototypeOf, parseFloat = context.parseFloat, pow = Math.pow, propertyIsEnumerable = objectProto.propertyIsEnumerable, setTimeout = context.setTimeout, splice = arrayProto.splice;
        /* Native method references for those with the same name as other `lodash` methods. */
        var nativeCeil = Math.ceil, nativeCreate = getNative(Object, 'create'), nativeFloor = Math.floor, nativeIsFinite = context.isFinite, nativeKeys = Object.keys, nativeMax = Math.max, nativeMin = Math.min, nativeParseInt = context.parseInt, nativeRandom = Math.random;
     ...
     */
        function arraySum(array, iteratee) {
            var index = -1, length = array.length, result = 0;
            while (++index < length) {
     ...
     */
        function assignValue(object, key, value) {
            ...
            if ((value === value ? value !== oldValue : oldValue === oldValue) || value === undefined && !(key in object)) {
                object[key] = value;
            }
        }
        /**
     ...
     */
        function assignMergeValue(object, key, value) {
            var oldValue = object[key];
            if (value !== undefined && (value === value ? value !== oldValue : oldValue === oldValue) || typeof key == 'number' && value === undefined && !(key in object)) {
                object[key] = value;
            }
        }
        /**
     ...
     */
        function baseFlatten(array, isDeep, isStrict, result) {
            ...
            while (++index < length) {
                var value = array[index];
                if (isObject(value) && isArrayLike(value) && (isStrict || isArray(value) || isArguments(value))) {
                    if (isDeep) {
        ...
        function baseIsMatch(object, source, matchData, customizer) {
            ...
            while (++index < length) {
                ...
                if (noCustomizer && data[2]) {
                    ...
                    var stackA = [], stackB = [], result = customizer ? customizer(objValue, srcValue, key, object, source, stackA, stackB) : undefined;
     ...
     */
        ...
        if (enumerate && !propertyIsEnumerable.call({ 'valueOf': 1 }, 'valueOf')) {
            baseKeysIn = function (object) {
                var data, iterator = enumerate(object), result = [];
     ...
     */
        function baseMatchesProperty(path, srcValue) {
            return function (object) {
                ...
                return objValue === undefined && objValue === srcValue ? hasIn(object, path) : baseIsEqual(srcValue, objValue, undefined, true);
            };
        }
        /**
     ...
     */
        function baseMergeDeep(object, source, key, mergeFunc, customizer, stackA, stackB) {
            var length = stackA.length, oldValue = object[key], srcValue = source[key];
            while (length--) {
            ...
            var newValue = customizer ? customizer(oldValue, srcValue, key + '', object, source, stackA, stackB) : undefined, isCommon = newValue === undefined;
            if (isCommon) {
                newValue = srcValue;
                if (isArray(srcValue) || isTypedArray(srcValue)) {
                    newValue = isArray(oldValue) ? oldValue : isObject(oldValue) && isArrayLike(oldValue) ? copyArray(oldValue) : [];
                } else if (isPlainObject(srcValue) || isArguments(srcValue)) {
                    newValue = isArguments(oldValue) ? toPlainObject(oldValue) : isPlainObject(oldValue) ? oldValue : {};
                } else {
            ...
            while (length--) {
                ...
                if (lastIndex == length || index != previous) {
                    ...
                    if (isIndex(index)) {
                        ...
                        var path = toPath(index), object = parent(array, path);
                        if (object != null) {
     ...
     */
        function baseSortByOrder(collection, iteratees, orders) {
            var toIteratee = getIteratee(), index = -1;
     ...
     */
        function createAssigner(assigner) {
            return restParam(function (object, sources) {
                var index = -1, length = object == null ? 0 : sources.length, customizer = length > 1 ? sources[length - 1] : undefined, guard = length > 2 ? sources[2] : undefined;
     ...
     */
        function createBaseFor(fromRight) {
            return function (object, iteratee, keysFunc) {
                var iterable = Object(object), props = keysFunc(object), length = props.length, index = fromRight ? length : -1;
                while (fromRight ? index-- : ++index < length) {
                    var key = props[index];
        ...
        function createHybridWrapper(func, bitmask, thisArg, partials, holders, partialsRight, holdersRight, argPos, ary, arity) {
            ...
            function wrapper() {
                ...
                if (isCurry || isCurryRight) {
                    ...
                    if (length < arity) {
                        var newArgPos = argPos ? copyArray(argPos) : undefined, newArity = nativeMax(arity - length, 0), newsHolders = isCurry ? argsHolders : undefined, newHoldersRight = isCurry ? undefined : argsHolders, newPartials = isCurry ? args : undefined, newPartialsRight = isCurry ? undefined : args;
     ...
     */
        function equalArrays(array, other, equalFunc, customizer, isLoose, stackA, stackB) {
            ...
            while (++index < arrLength) {
                ...
                if (customizer) {
                    var result = isLoose ? customizer(othValue, arrValue, index, other, array, stackA, stackB) : customizer(arrValue, othValue, index, array, other, stackA, stackB);
                }
                if (result !== undefined) {
                    if (result) {
                        continue;
     ...
     */
        function equalObjects(object, other, equalFunc, customizer, isLoose, stackA, stackB) {
            ...
            while (++index < objLength) {
                ...
                if (customizer) {
                    var result = isLoose ? customizer(othValue, objValue, key, other, object, stackA, stackB) : customizer(objValue, othValue, key, object, other, stackA, stackB);
                }
     ...
     */
        function hasPath(object, path, hasFunc) {
            ...
            return result || isLength(object && object.length) && isIndex(path, object.length) && (isArray(object) || isArguments(object) || isString(object));
        }
        /**
     ...
     */
        function isArrayLike(value) {
            return value != null && !(typeof value == 'function' && objToString.call(value) == funcTag) && isLength(getLength(value));
        }
        /**
     ...
     */
        var difference = restParam(function (array, values) {
                return isObject(array) && isArrayLike(array) ? baseDifference(array, baseFlatten(values, false, true)) : [];
            });
        /**
     ...
     */
        function dropRightWhile(array, predicate) {
            return array && array.length ? baseWhile(array, getIteratee(predicate), true, true) : [];
        }
        /**
     ...
     */
        function dropWhile(array, predicate) {
            return array && array.length ? baseWhile(array, getIteratee(predicate), true) : [];
        }
        /**
     ...
     */
        function findIndex(array, predicate) {
            return array && array.length ? baseFindIndex(array, getIteratee(predicate)) : -1;
        }
        /**
     ...
     */
        function findLastIndex(array, predicate) {
            return array && array.length ? baseFindIndex(array, getIteratee(predicate), true) : -1;
        }
        /**
     ...
     */
        function takeRightWhile(array, predicate) {
            return array && array.length ? baseWhile(array, getIteratee(predicate), false, true) : [];
        }
        /**
     ...
     */
        function takeWhile(array, predicate) {
            return array && array.length ? baseWhile(array, getIteratee(predicate)) : [];
        }
        /**
     ...
     */
        function uniq(array, isSorted) {
            ...
            return isSorted && typeof isSorted == 'boolean' && getIndexOf() === baseIndexOf ? sortedUniq(array) : baseUniq(array);
        }
        /**
     ...
     */
        var without = restParam(function (array, values) {
                return isObject(array) && isArrayLike(array) ? baseDifference(array, values) : [];
            });
        /**
     ...
     */
        function reduce(collection, iteratee, accumulator) {
            ...
            return typeof iteratee == 'function' && isArray(collection) ? arrayReduce(collection, iteratee, accumulator, initFromArray) : baseReduce(collection, getIteratee(iteratee), accumulator, initFromArray, baseEach);
        }
        /**
     ...
     */
        function reduceRight(collection, iteratee, accumulator) {
            var initFromArray = arguments.length < 3;
            return typeof iteratee == 'function' && isArray(collection) ? arrayReduceRight(collection, iteratee, accumulator, initFromArray) : baseReduce(collection, getIteratee(iteratee), accumulator, initFromArray, baseEachRight);
        }
        /**
     ...
     */
        function isPlainObject(value) {
            ...
            var proto = typeof value.constructor == 'function' ? getPrototypeOf(value) : objectProto;
            if (proto === null) {
                return true;
            }
     ...
     */
        function keys(object) {
            ...
            var result = initKeys(object), length = result.length, skipIndexes = !!length;
            for (var key in object) {
                if (hasOwnProperty.call(object, key) && !(skipIndexes && isIndex(key, length)) && !(isProto && key == 'constructor')) {
                    result.push(key);
                }
            }
            return result;
     ...
     */
        function keysIn(object) {
            object = Object(object);
            var index = -1, isProto = isPrototype(object), props = baseKeysIn(object), propsLength = props.length, result = initKeys(object), length = result.length, skipIndexes = !!length;
     ...
     */
        function endsWith(string, target, position) {
            ...
            position = position === undefined ? length : nativeMin(position < 0 ? 0 : toInteger(position), length);
     ...
     */
        function startsWith(string, target, position) {
            string = baseToString(string);
            position = position == null ? 0 : nativeMin(position < 0 ? 0 : toInteger(position), string.length);
     ...
     */
        function template(string, options, otherOptions) {
            ...
            var imports = extendWith({}, options.imports, settings.imports, extendDefaults), importsKeys = keys(imports), importsValues = baseValues(imports, importsKeys);
     ...
     */
        function max(array) {
            return array && array.length ? arrayExtremum(array, identity, gt, NEGATIVE_INFINITY) : NEGATIVE_INFINITY;
        }
        /**
     ...
     */
        function maxBy(array, iteratee) {
            return array && array.length ? arrayExtremum(array, getIteratee(iteratee), gt, NEGATIVE_INFINITY) : NEGATIVE_INFINITY;
        }
        /**
     ...
     */
        function min(array) {
            return array && array.length ? arrayExtremum(array, identity, lt, POSITIVE_INFINITY) : POSITIVE_INFINITY;
        }
        /**
     ...
     */
        function minBy(array, iteratee) {
            return array && array.length ? arrayExtremum(array, getIteratee(iteratee), lt, POSITIVE_INFINITY) : POSITIVE_INFINITY;
        }
        /**
     ...
     */
        function sumBy(array, iteratee) {
            return array && array.length ? arraySum(array, getIteratee(iteratee)) : 0;
        }
        /*------------------------------------------------------------------------*/
        ...
        baseForOwn(LazyWrapper.prototype, function (func, methodName) {
            ...
            lodash.prototype[methodName] = function () {
                ...
                var action = {
                        ...
                        'args': [interceptor],
                        'thisArg': undefined
                    }, chainAll = this.__chain__, isHybrid = !!this.__actions__.length, isUnwrapped = retUnwrapped && !chainAll, onlyLazy = isLazy && !isHybrid;
